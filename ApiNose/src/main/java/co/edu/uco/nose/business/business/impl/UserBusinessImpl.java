	...
	public void registerNewUserInformation(UserDomain userDomain) {
		...
		private void validateUserDataConsistency (final UserDomain userDomain) {
		    ...
		    if (ObjectHelper.isNull(userDomain.getIdentificationType())) {
		        var userMessage = MessagesEnum.USER_ERROR_WHEN_REGISTERING_USER_NULL_IDENTIFICATION_TYPE.getContent();
		        var technicalMessage = MessagesEnum.TECHNICAL_ERROR_WHEN_REGISTERING_USER_NULL_IDENTIFICATION_TYPE.getContent();
		        throw NoseException.create(userMessage, technicalMessage);
		    }

		    if (ObjectHelper.isNull(userDomain.getResidenceCity())) {
		        var userMessage = MessagesEnum.USER_ERROR_WHEN_REGISTERING_USER_NULL_CITY.getContent();
		        var technicalMessage = MessagesEnum.TECHNICAL_ERROR_WHEN_REGISTERING_USER_NULL_CITY.getContent();
		        throw NoseException.create(userMessage, technicalMessage);
		    }

		    if (TextHelper.isEmptyWithTrim(userDomain.getIdentificationNumber()) ||
		        userDomain.getIdentificationNumber().length() > 25) {
		        var userMessage = MessagesEnum.USER_ERROR_WHEN_REGISTERING_USER_INVALID_IDENTIFICATION_NUMBER.getContent();
		        var technicalMessage = MessagesEnum.TECHNICAL_ERROR_WHEN_REGISTERING_USER_INVALID_IDENTIFICATION_NUMBER.getContent();
		        throw NoseException.create(userMessage, technicalMessage);
		    }
		    ...
		    if (TextHelper.isEmptyWithTrim(userDomain.getMiddleName()) ||
		        ...
		        var userMessage = MessagesEnum.USER_ERROR_WHEN_REGISTERING_USER_INVALID_MIDDLE_NAME.getContent();
		        var technicalMessage = MessagesEnum.TECHNICAL_ERROR_WHEN_REGISTERING_USER_INVALID_MIDDLE_NAME.getContent();
		        throw NoseException.create(userMessage, technicalMessage);
		    }
		    ...
		    if (TextHelper.isEmptyWithTrim(userDomain.getSecondLastName()) ||
		        ...
		        var userMessage = MessagesEnum.USER_ERROR_WHEN_REGISTERING_USER_INVALID_SECOND_LAST_NAME.getContent();
		        var technicalMessage = MessagesEnum.TECHNICAL_ERROR_WHEN_REGISTERING_USER_INVALID_SECOND_LAST_NAME.getContent();
		        throw NoseException.create(userMessage, technicalMessage);
		    }
		    ...
		    if (!TextHelper.isValidPhoneNumber(userDomain.getCellPhoneNumber()) ||
		        ...
		        var userMessage = MessagesEnum.USER_ERROR_WHEN_REGISTERING_USER_INVALID_CELL_PHONE_NUMBER.getContent();
		        var technicalMessage = MessagesEnum.TECHNICAL_ERROR_WHEN_REGISTERING_USER_INVALID_CELL_PHONE_NUMBER.getContent();
		        throw NoseException.create(userMessage, technicalMessage);
		    }
		}
		    private void validateDuplicateUser (final UserDomain userDomain) {
			    ...
			    filterByIdentification.setIdType(userEntityFilter.getIdentificationType());
